...
async function init () {
    const { mode, readOnly, value, options } = props
...
### `useWindowSizeFn.js` 窗口改变监听hooks
...
传入JSON数据value，可以通过`JSON.stringify(data, null, 2)`转成带换行的JSON格式数据

```HTML
<codeMirror :value="data" ref="codeMirrorRef"></codeMirror>
```
